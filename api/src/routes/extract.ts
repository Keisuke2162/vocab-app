...
extract.post("/words", async (c) => {
  ...
  const message = await client.messages.create({
    ...
    max_tokens: 8192,
    system:
      "あなたは英語教育のスペシャリストです。中学英語レベルはわかるが、それ以上の語彙・熟語・表現は知らない日本語話者向けに、英語記事から学習すべき語彙を抽出します。専門的な記事も扱います。指定のJSON形式のみで返してください。",
    messages: [
      {
        role: "user",
        content: `以下の英語記事から、英語学習に役立つ単語・熟語・表現を抽出してください。\n\n【抽出ルール】\n- 対象: 中学英語レベルを超える単語・熟語・技術用語・表現（"implement", "refactor", "breaking change", "under the hood" のようなプログラミング文脈の語彙も含む）\n- 表現は4語程度まで（クイズ用に短くする）\n- 除外: 中学英語レベルの基本単語（"use", "make", "run" など）・コードとして書かれたもの（コードブロック・変数名・関数呼び出しなど）・完全な文章・URL・固有名詞\n- 日本語訳は簡潔に（例: "implement" → "実装する"、"breaking change" → "後方互換性を壊す変更"）\n\n説明文や前置きは不要です。JSONのみ返してください。\n\n{\n  "words": [\n    { "en": "...", "ja": "..." }\n  ]\n}\n\n記事:\n${body.text}`,
      },
    ],
  });
