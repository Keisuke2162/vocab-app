...
extract.post("/words", async (c) => {
  ...
  const message = await client.messages.create({
    ...
    max_tokens: 8192,
    system:
      "あなたは英語教育のスペシャリストです。英語学習者向けに、英語記事から語彙・熟語を抽出します。指定のJSON形式のみで返してください。",
    messages: [
      {
        role: "user",
        content: `以下の英語記事から、英語学習に役立つ単語・熟語を抽出してください。\n\n【抽出ルール】\n- 対象: 英単語・熟語・技術用語（"implement", "refactor", "breaking change", "under the hood" のようなプログラミング文脈の語彙も含む）\n- 除外: コードとして書かれたもの（コードブロック・変数名・関数呼び出し・演算子など）・完全な文章・URL・固有名詞\n- 日本語訳は簡潔に（例: "implement" → "実装する"）\n\n説明文や前置きは不要です。JSONのみ返してください。\n\n{\n  "words": [\n    { "en": "...", "ja": "..." }\n  ]\n}\n\n記事:\n${body.text}`,
      },
    ],
  });
