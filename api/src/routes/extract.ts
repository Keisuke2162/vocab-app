...
extract.post("/words", async (c) => {
  ...
  const message = await client.messages.create({
    ...
    messages: [
      {
        role: "user",
        content: `以下の英語記事から、wordsとexpressionsを分けて漏れなく抽出してください。\n\n【words】中級以上の単語・専門用語\n- 除外: 超基本単語（go, get, make, have, be等）・コード・固有名詞\n\n【expressions】記事中に登場する全ての句動詞・熟語・接続表現・前置詞句\n- 例: "result in", "allow for", "given that", "as part of", "in the meantime", "conform to", "lay out", "for instance", "more generally", "make it difficult to", "not only A but B", "in such a way as to", "as is typically seen in"\n- 4語程度まで。記事に出てくる表現は種類を問わず全て含める\n\n【日本語訳】簡潔に\n\n説明文や前置きは不要です。JSONのみ返してください。\n\n{\n  "words": [ { "en": "...", "ja": "..." } ],\n  "expressions": [ { "en": "...", "ja": "..." } ]\n}\n\n記事:\n${body.text}`,
      },
    ],
  });
