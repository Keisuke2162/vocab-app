...
extract.post("/words", async (c) => {
  ...
  const message = await client.messages.create({
    ...
    max_tokens: 8192,
    system:
      "あなたは英語教育のスペシャリストです。英語学習者向けに、英語記事から語彙・熟語を抽出します。抽出対象は英単語・熟語のみです。文章・フレーズ・コード・プログラミング構文・固有名詞・URLは含めないでください。指定のJSON形式のみで返してください。",
    messages: [
      {
        role: "user",
        content: `以下の英語記事から、英語学習に役立つ単語・熟語を抽出してください。\n\n【抽出ルール】\n- 対象: 英単語・熟語（2〜4語程度のイディオムまで）\n- 除外: 文章・長いフレーズ・コードスニペット・プログラミング構文（関数名・メソッド名・変数名など）・固有名詞・URL\n- 日本語訳は簡潔に（例: "implement" → "実装する"）\n\n説明文や前置きは不要です。JSONのみ返してください。\n\n{\n  "words": [\n    { "en": "...", "ja": "..." }\n  ]\n}\n\n記事:\n${body.text}`,
      },
    ],
  });
